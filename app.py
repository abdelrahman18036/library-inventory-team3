...
import json
import os
from prometheus_client import Counter, generate_latest, Summary, Gauge, start_http_server
...
def add_book():
    ...
    REQUEST_COUNT.labels(method=request.method, endpoint='/add_book').inc()
    with REQUEST_LATENCY.labels(method=request.method, endpoint='/add_book').time():
        IN_PROGRESS.labels(endpoint='/add_book').inc()
        try:
            ...
            IN_PROGRESS.labels(endpoint='/add_book').dec()

@app.route('/update_book/<title>', methods=['GET', 'POST'])
def update_book(title):
    """Handle the update of book details."""
    REQUEST_COUNT.labels(method=request.method, endpoint='/update_book/<title>').inc()
    with REQUEST_LATENCY.labels(method=request.method, endpoint='/update_book/<title>').time():
        IN_PROGRESS.labels(endpoint='/update_book/<title>').inc()
        try:
            data = load_data()
            ...
            IN_PROGRESS.labels(endpoint='/update_book/<title>').dec()

@app.route('/remove_book', methods=['GET', 'POST'])
def remove_book():
    """Handle the removal of books."""
    REQUEST_COUNT.labels(method=request.method, endpoint='/remove_book').inc()
    with REQUEST_LATENCY.labels(method=request.method, endpoint='/remove_book').time():
        IN_PROGRESS.labels(endpoint='/remove_book').inc()
        try:
            if request.method == 'POST':
                title = request.form['title']
                data = load_data()
            ...
            IN_PROGRESS.labels(endpoint='/remove_book').dec()

@app.route('/search_book', methods=['GET', 'POST'])
def search_book():
    """Handle searching for books by title."""
    REQUEST_COUNT.labels(method=request.method, endpoint='/search_book').inc()
    with REQUEST_LATENCY.labels(method=request.method, endpoint='/search_book').time():
        IN_PROGRESS.labels(endpoint='/search_book').inc()
        try:
            if request.method == 'POST':
                title = request.form['title']
                if not title:
            ...
            IN_PROGRESS.labels(endpoint='/search_book').dec()

@app.route('/borrow_book', methods=['GET', 'POST'])
def borrow_book():
    """Handle borrowing books."""
    REQUEST_COUNT.labels(method=request.method, endpoint='/borrow_book').inc()
    with REQUEST_LATENCY.labels(method=request.method, endpoint='/borrow_book').time():
        IN_PROGRESS.labels(endpoint='/borrow_book').inc()
        try:
            data = load_data()
            if request.method == 'POST':
                title = request.form['title']
            ...
            IN_PROGRESS.labels(endpoint='/borrow_book').dec()

@app.route('/return_book', methods=['GET', 'POST'])
def return_book():
    """Handle returning borrowed books."""
    REQUEST_COUNT.labels(method=request.method, endpoint='/return_book').inc()
    with REQUEST_LATENCY.labels(method=request.method, endpoint='/return_book').time():
        IN_PROGRESS.labels(endpoint='/return_book').inc()
        try:
            data = load_data()
            ...
            IN_PROGRESS.labels(endpoint='/return_book').dec()
